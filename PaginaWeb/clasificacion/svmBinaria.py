...
st.header("Ejemplos separables linealmente")
...
Se parte de un conjunto de ejemplos de entrenamiento etiquetados: $ \left\{ (x_{1}, y_{1}) , \cdots , (x_{n},y_{n})\right\}$, 
...
Si el conjunto de ejemplos de entrenamiento son linealmente separables en dos clases $\left\{ -1,+1\right\}$, entonces 
...
$$
 ...
 de modo que divide el conjunto de ejemplos de entrenamiento en dos clases:  
...
Para los ejemplos más cercanos al hiperplano, que delimitan el margen de cada lado y que se denominan 
...
En resumen, cuando los ejemplos son separables linealmente se elige como hiperplano de separación el que equidista de los dos ejemplos más cercanos de cada una de las dos clases. Además, los parámetros del hiperplano se determinan teniendo en cuenta únicamente los vectores soporte (aquellos $(x_{i},y_{i})$ en los que $y_{i}(\omega^{t}x_{i} + b)=1$).

En la práctica el conjunto de ejemplos de entrenamiento raramente va a ser linealmente separables debido a la existencia de ruido.
